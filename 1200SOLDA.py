...
class ControlMotor:
    def __init__(self, root):
        ...
        # Variables de cámara - VIDEO 720p
        ...
        # Verificar cámara e iniciar video
        self.verificar_camara()
        self.inicializar_camara_video()
    ...
    def verificar_camara(self):
        """Verifica que rpicam-vid esté disponible"""
        try:
            # Verificar si rpicam-vid existe
            comando = self.encontrar_comando_video()
            if comando:
                self.camara_disponible = True
                self.lbl_estado_camara.configure(text="📷 Cámara lista", text_color="#2ecc71")
                self.log(f"✅ Cámara disponible ({comando})", "OK")
            else:
                self.camara_disponible = False
                self.lbl_estado_camara.configure(text="❌ Cámara no disponible", text_color="#e74c3c")
                self.log("❌ rpicam-vid no instalado", "ERROR")
        except:
            self.camara_disponible = False
            self.lbl_estado_camara.configure(text="❌ Cámara no disponible", text_color="#e74c3c")
            self.log("❌ rpicam-vid no instalado", "ERROR")
    
    def encontrar_comando_video(self):
        """Encuentra el comando de video disponible"""
    ...
    def inicializar_camara_video(self):
        """Inicializa la cámara en modo video 720p"""
        ...
        try:
            ...
            self.log("📷 Video 720p iniciado", "OK")
        except Exception as e:
            self.log(f"❌ Error iniciando video: {e}", "ERROR")
    
    def capturar_video_720p(self):
        """Captura video en 720p"""
        comando = self.encontrar_comando_video()
        if comando is None:
            self.camara_activa = False
            return
                ...
                try:
                    ...
                    while len(buffer) >= frame_size:
                        ...
                        try:
                            ...
                            yuv = yuv.reshape((720 * 3 // 2, 1280))
                            frame_bgr = cv2.cvtColor(yuv, cv2.COLOR_YUV2BGR_I420)
                            
                            self.frame = frame_bgr.copy()
                            
                            # Redimensionar al tamaño de la columna de video
                            frame_display = cv2.resize(frame_bgr, (self.video_width, self.video_height))
                    ...
                    if self.camara_activa:
                        self.log("⚠️ Error captura", "WARNING")
                    break
    ...
    def actualizar_fps(self):
        ...
        self.lbl_fps.configure(text=f"{self.fps} FPS")
    
    def capturar_frame_720p(self, ruta):
        """Captura el frame actual del video y lo guarda"""
        try:
            if self.frame is None:
                self.log("⚠️ No hay frame disponible", "WARNING")
                return False
            
            # Guardar frame
            cv2.imwrite(ruta, self.frame, [cv2.IMWRITE_JPEG_QUALITY, 95])
            self.log(f"✅ Frame guardado: {os.path.basename(ruta)}", "FOTO")
            return True
        except Exception as e:
            self.log(f"❌ Error guardando frame: {e}", "ERROR")
            return False
    ...
    def toggle_camara(self):
        ...
        if self.camara_activa:
            ...
            self.lbl_estado_camara.configure(text="📷 Cámara OFF", text_color="gray")
            self.foto_label.configure(image=None, text="📷 Cámara detenida")
            self.foto_label.image = None
            self.log("📷 Cámara detenida", "INFO")
            ...
            self.lbl_estado_camara.configure(text="📷 Cámara ON", text_color="#2ecc71")
            self.log("📷 Iniciando cámara...", "INFO")
            self.inicializar_camara_video()
    ...
    def crear_interfaz(self):
        ...
        self.lbl_fps = ctk.CTkLabel(frame_superior, text="0 FPS", font=ctk.CTkFont(size=9), text_color="gray")
        self.lbl_fps.pack(side=ctk.RIGHT, padx=5)
        ...
        self.btn_camara = ctk.CTkButton(
            frame_botones_hs, text="📷 ON", command=self.toggle_camara,
            height=30, fg_color="#2ecc71", font=ctk.CTkFont(size=10, weight="bold")
        )
        self.btn_camara.pack(side=ctk.LEFT, padx=2, fill=ctk.X, expand=True)
        ...
        ctk.CTkLabel(frame_titulo_foto, text="📸 Video 720p", font=ctk.CTkFont(size=10, weight="bold")).pack(side=ctk.LEFT, padx=3)
        
        # Botón capturar frame
        self.btn_capturar_frame = ctk.CTkButton(
            frame_titulo_foto,
            text="📷 CAPTURAR",
            command=self.capturar_frame_manual,
            height=25,
            width=80,
            fg_color="#e67e22",
            font=ctk.CTkFont(size=9, weight="bold")
        )
        self.btn_capturar_frame.pack(side=ctk.RIGHT, padx=3)
    ...
    def capturar_frame_manual(self):
        """Captura el frame actual manualmente"""
        if not self.camara_activa:
            self.log("⚠️ Cámara no activa", "WARNING")
            return
        ...
        carpeta_completa = os.path.join(self.carpeta_fotos, nombre_carpeta)
        os.makedirs(carpeta_completa, exist_ok=True)
        
        # Guardar frame con timestamp
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        ruta = os.path.join(carpeta_completa, f"frame_{timestamp}.jpg")
        
        if self.capturar_frame_720p(ruta):
            self.mostrar_ultima_captura(ruta)
            self.log(f"✅ Frame guardado en: {ruta}", "FOTO")
    
    def mostrar_ultima_captura(self, ruta):
        """Muestra la última captura en la interfaz"""
        try:
            img = Image.open(ruta)
            ...
            self.foto_label.configure(image=imgtk, text="")
            self.foto_label.image = imgtk
            self.ultima_foto_path = ruta
            self.lbl_ultima_foto.configure(text=f"Última: {os.path.basename(ruta)}")
            return True
        except:
            return False
    ...
    def iniciar_inspeccion(self):
        ...
        try:
            ...
            # Crear carpeta sin timestamp (solo el nombre)
            nombre_carpeta = self.entry_carpeta.get().strip() or "inspeccion"
            self.carpeta_actual = os.path.join(self.carpeta_fotos, nombre_carpeta)
            ...
            self.log(f"📁 Carpeta: {nombre_carpeta}", "INFO")
            ...
            self.btn_capturar_frame.configure(state="disabled")
            self.inspeccion_activa = True
            
            self.enviar_comando("INSPECCION_START")
            time.sleep(0.2)
            
            threading.Thread(target=self.proceso_inspeccion_video, daemon=True).start()
            
        except Exception as e:
            self.log(f"❌ Error: {e}", "ERROR")

    def proceso_inspeccion_video(self):
        """Proceso de inspección usando frames del video"""
        try:
            self.log(f"📁 Carpeta: {os.path.basename(self.carpeta_actual)}", "INFO")
            ...
            for i, pasos_bloque in enumerate(self.pasos_por_bloque):
                ...
                ruta_completa = os.path.join(self.carpeta_actual, nombre)
                
                self.log(f"📸 Capturando frame {i+1}/{self.total_fotos}", "FOTO")
                self.actualizar_estado_visual(f"📸 Capturando frame {i+1}...")
                
                # Capturar frame del video
                if self.capturar_frame_720p(ruta_completa):
                    self.fotos_tomadas = i + 1
                    self.root.after(0, self.mostrar_ultima_captura, ruta_completa)
                    self.root.after(0, lambda n=nombre: self.lbl_ultima_foto.configure(text=f"Última: {n}"))
                else:
                    self.log(f"❌ Error capturando frame {i+1}", "ERROR")
            ...
            self.root.after(0, lambda: self.btn_capturar_frame.configure(state="normal"))
            self.consultar_estado()
